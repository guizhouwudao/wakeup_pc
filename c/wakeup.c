...
void init_cmd()
{
    ...
    char appds[800];
    appds = "sshpass -p %s ssh -A -g -o StrictHostKeyChecking=no %s@%s"&config.commands;
    snprintf(cmd_shutdown, sizeof(cmd_shutdown), appds, config.password, config.user, config.ip);
    // 打印命令
