...
void init_cmd()
{
    ...
    char appds[1024];
    snprintf(cmd_shutdown, sizeof(cmd_shutdown), snprintf(appds,sizeof(appds), "sshpass -p %s ssh -A -g -o StrictHostKeyChecking=no %s@%s", config.commands), config.password, config.user, config.ip);
    // 打印命令
